...
/* ── GET /api/items?q=leftovers&limit=20 ── */
router.get('/', (req, res) => {
  const db = getDb();
...
router.get('/:id', (req, res) => {
  const db  = getDb();
  const row = db.prepare('SELECT * FROM items WHERE id = ?').get(req.params.id.toLowerCase());
  if (!row) return res.status(404).json({ error: 'Objeto no encontrado' });
  res.json(row);
});
